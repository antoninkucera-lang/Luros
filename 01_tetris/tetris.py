...
# seznam pozic dopadlých bloků
dopadle_bloky = []

# čas posledního pádu a rychlost pádu v milisekundách
cas_posledniho_padu = pygame.time.get_ticks()
rychlost_padu = 500  # blok spadne o jedno pole každých 500ms

# funkce nakreslí mřížku přes celé okno
def mrizka():
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT and blok_x > 0:
                blok_x -= 1
            if event.key == pygame.K_RIGHT and blok_x < sloupce - 1:
                blok_x += 1
            if event.key == pygame.K_DOWN:
                blok_y += 1

    # automatický pád bloku podle časovače
    ...
    nakresli_dopadle()
    # zobrazení snímku a omezení na 60 FPS
    pygame.display.flip()
    hodiny.tick(60)
